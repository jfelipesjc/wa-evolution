...
- **Communities** — `create`, `findCommunity`, `updateSubject`,
  `updateDescription`, `linkGroup`, `unlinkGroup`, `linkedGroups`,
  `requestList`, `requestUpdate` (approve/reject), `updateParticipant`
...
- **Newsletters / Channels** — `create`, `follow`, `unfollow`, `findNewsletter`,
  ...
  `reactionMode`, `fetchMessages`, `adminCount`, `changeOwner`, `demote`,
  `subscribeUpdates` (all under `/newsletter/<op>/{i}`)
